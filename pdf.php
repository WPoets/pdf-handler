...
\aw2_library::add_service("pdf.generate", "PDF Generate", [
...
{
    if (
        \aw2_library::pre_actions("all", $atts, $content, $shortcode) == false
    ) {
        return;
    }
    ...
    $return_value = call_user_func([$aw2_pdf, $part]);
...
class awesome2_pdf_handler
{
    ...
    function download()
    {
        ...
        if (empty($pdf["filename"])) {
            \aw2_library::set_error(
                "filename is required for part=download aw2.pdf",
            );
            return \aw2_library::get("errors");
        }
    ...
    function save()
    {
        ...
        if (empty($pdf["filename"])) {
            \aw2_library::set_error(
                "filename is required for part=save aw2.pdf",
            );
            return \aw2_library::get("errors");
        }
    ...
    function app_option()
    {
        $app_setting_sections = &\aw2_library::get_array_ref(
        ...
        if ($part == "field") {
            ...
            if (!array_key_exists("fields", $app_setting_sections[$key])) {
                $app_setting_sections[$key]["fields"] = [];
            }
...
\aw2_library::add_service("pdf.parse", "To parse the PDF Acro or XFA form", [
    "namespace" => __NAMESPACE__,
]);

function parse($atts, $content = null, $shortcode = null)
{
    if (
        \aw2_library::pre_actions("all", $atts, $content, $shortcode) == false
    ) {
